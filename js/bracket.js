...
(function ($) {
      ...
      if (!this.name.isEmpty()) {
        ...
        try {
          ...
          if (e instanceof EndOfBranchException) {
            ...
            throw new Error(
              'Unexpected exception type (message: "'.concat(e.message, '")')
            );
          }
        }
      }
  ...
  function winnerBubbles(match) {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem("winner", winner.text());
      winner.append('<div class="bubble">1º</div>');
      var loser = el.find(".team.lose");
      localStorage.setItem("vice", loser.text());
  ...
  function consolationBubbles(match) {
    ...
    setTimeout(() => {
      var winner = el.find(".team.win");
      localStorage.setItem("consolation", winner.text());
  ...
  var Round = /** @class */ (function () {
    function Round(
      ...
      this.opts = opts;
      this.containerWidth = this.opts.disableScoring
        ? this.opts.teamWidth
        : this.opts.teamWidth + this.opts.scoreWidth;
      this.roundCon = $(
        '<div class="round" style="width: '
          .concat(this.containerWidth, "px; margin-right: ")
          .concat(this.opts.roundMargin, 'px"/>')
      );
      this.matches = [];
    }
    ...
    Round.prototype.size = function () {
      let divisor = this.opts.twoBrackets ? 2 : 1;
      let length = this.matches.length / divisor;
      return length;
    };
    Round.prototype.render = function () {
      ...
      this.matches.forEach(function (m, _i) {
        return m.render();
      });
    };
  ...
  var Bracket = /** @class */ (function () {
    function Bracket(bracketCon, initResults, mkMatch, isFirstBracket, opts) {
      this.brackets = bracketCon;
      this.bracketCon = this.brackets.eq(0);
    ...
    Bracket.prototype.eq = function (n) {
      this.bracketCon = this.brackets.eq(n);
    };
  ...
  function createConnector(roundMargin, connector, align) {
    ...
    var _c = calculateHeight(connector.height),
      height = _c.height,
      drop = _c.drop;
    ...
    var doShift = shift >= 0;
    var src = $('<div class="connector"></div>').css(
      ((_a = {}),
      (_a[align] = -width - 2),
    ...
    var dst = $('<div class="connector"></div>').css(
      ((_b = {}),
      (_b[align] = -width),
      (_b.bottom = drop ? 0 : ""),
      (_b.top = !drop ? 0 : ""),
  ...
  function createTeam(
    ...
    var sEl = $(
      '<div class="score" style="width: '
        .concat(opts.scoreWidth, 'px;" ')
        .concat(resultIdAttribute, ">")
    );
    var score =
    ...
    var tEl = $(
      '<div class="team" style="width: '.concat(
        opts.disableScoring ? opts.teamWidth : opts.teamWidth + opts.scoreWidth,
        'px;"></div>'
      )
    );
    var nEl = $(
      '<div class="label" style="width: '.concat(
        opts.disableScoring ? opts.teamWidth : opts.teamWidth + opts.scoreWidth,
        'px;"></div>'
      )
    ).appendTo(tEl);
      ...
      if (!opts.disableTeamEdit) {
        ...
        tEl.click(function () {
          if (
            ...
            team.score = Option.of(
              opts.extension.evaluateScore("1", team.score.toNull())
            );
            team.sibling().score = Option.of(
      ...
      if (
        ...
        sEl.click(function () {
          ...
          function editor() {
            ...
            input.blur(function () {
              var val = opts.extension.evaluateScore(
                input.val(),
                team.score.toNull()
              );
              if (val === null) {
  ...
  var Match = /** @class */ (function () {
    function Match(
      ...
      if (
        ...
        console.warn(
          "ERROR IN SCORE DATA: "
            .concat(match.a.source().name, ": ")
            .concat(match.a.score, ", ")
            .concat(match.b.source().name, ": ")
            .concat(match.b.score)
        );
    ...
    Match.prototype.connect = function (cb) {
      ...
      if (this.seed >= this.round.matches.length / 2 && this.opts.twoBrackets) {
        align = "left";
      }
      let roundCount = this.round.bracket.rounds.length;
      if (this.round.roundNumber == roundCount - 2 && this.opts.twoBrackets) {
        matchupOffset = 0; ///Testar
      }
    ...
    Match.prototype.render = function () {
      ...
      if (
        this.seed == this.round.matches.length / 2 &&
        !isLast &&
        this.opts.twoBrackets &&
        this.isFirstBracket
      ) {
        this.round.bracket.eq(1);
        this.round.roundCon = this.round.roundCon.clone().empty();
        this.round.roundCon.appendTo(this.round.bracket.el);
      }
      ...
      if (isLast && this.opts.twoBrackets) height /= 2;
  ...
  var wrapResults = function (initResults) {
    return initResults.map(function (brackets) {
      return brackets.map(function (rounds) {
        return rounds.map(function (matches) {
          return new ResultObject(
            Option.of(undefinedToNull(matches[0])),
            Option.of(undefinedToNull(matches[1])),
            matches[2]
          );
        });
      });
  ...
  var JqueryBracket = function (opts) {
    ...
    var topCon = $('<div class="jQBracket ' + opts.dir + '"></div>');

    if (opts.twoBrackets) {
      topCon = topCon.add($('<div class="jQBracket rl"></div>'));
      height /= 2;
    }
    ...
    function resizeContainer() {
      ...
      topCon.each((_index, elm) => {
        var width = "fit-content";

        if (_index == 0 && !opts.disableToolbar)
          $(elm).css({
    ...
    if (!opts.disableToolbar) {
      topCon.first().append(createToolbar(data, opts));
    }
    var fEl;
    var wEl;
    var lEl;
  ...
  var getPositiveOrZero = function (expected) {
    ...
    if (value < expected) {
      throw new Error(
        "Value must be greater than ".concat(expected, ", got ").concat(value)
      );
    }
    return value;
  };
  ...
  function parseOptions(input, context, extension) {
    var opts = {
      ...
      dir: input.twoBrackets === true ? "lr" : parseDir(input.dir),
      ...
      twoBrackets: input.twoBrackets || false,
    };
    assertOptions(opts);
    return opts;
  }
  ...
  function parseInit(rawInit) {
    ...
    if (log2Result !== Math.floor(log2Result)) {
      $.error(
        '"teams" property must have 2^n number of team pairs, i.e. 1, 2, 4, etc. Got '.concat(
          value.teams.length,
          " team pairs."
        )
      );
  ...
  function init(ctx, originalOpts, extension) {
    ...
    if (disableToolbarGiven && disableToolbarType !== "boolean") {
      $.error(
        "disableToolbar must be a boolean, got ".concat(disableToolbarType)
      );
    }
